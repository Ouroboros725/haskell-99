...
countLeaves (Branch _ stl str) = countLeaves stl + countLeaves str

--61a
leaves :: Tree a -> [a]
leaves Empty = []
leaves (Branch x Empty Empty) = [x]
leaves (Branch _ stl str) = leaves stl ++ leaves str

--62
internals :: Tree a -> [a]
internals Empty = []
internals (Branch _ Empty Empty) = []
internals (Branch x stl str) = [x] ++ internals stl ++ internals str

--62b
atLevel :: Tree a -> Int -> [a]
atLevel tr t = atLevel' tr t 1
  where atLevel' Empty t l = []
        atLevel' (Branch x stl str) t l = if t == l then x : atLevel' stl t (l+1) ++ atLevel' str t (l+1)
                                                    else atLevel' stl t (l+1) ++ atLevel' str t (l+1)   

--63
completeBinaryTree :: Int -> Tree Char
completeBinaryTree 0 = Empty
completeBinaryTree 1 = Branch 'x' Empty Empty
completeBinaryTree n = let p = floor $ logBase 2 $ fromIntegral n
                           f = 2 ^ p
                           b = n - f + 1
                           l = take f $ foldr (\x acc -> if x <= b then Branch 'x' Empty Empty : acc else Empty : acc) [] [1..]
                       in head $ buildUpTree l
  where buildUpTree (b:[]) = [b]
        buildUpTree xs = buildUpTree $ buildUpLevel $ buildUpBranch xs
        buildUpBranch [] = []
        buildUpBranch (x:y:xs) = (x, y) : buildUpBranch xs 
        buildUpLevel xs = foldr (\(x, y) acc -> Branch 'x' x y : acc) [] xs                         
